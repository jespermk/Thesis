import numpy as np
from models.NN import ensemble_model, radial_VCL_image_model, MFG
...
em = ensemble_model(radials, device)
...
for e in t.arange(epoch_n):
    ...
    loss_ensemble += [loss.detach()]
    loss_ensemble = loss_ensemble[-ensemble_n:]

    # Binning data
    mean_loss_ensemble += [t.stack(loss_ensemble).mean().cpu()]
    em.to(device)
...
# Convert to numpy
mean_loss_ensemble = t.stack(mean_loss_ensemble).numpy()

# Save data
np.save('generated_data/mean_loss_ensemble', mean_loss_ensemble)
