...
def compute_posterior_stats(x, t, analyt, dim=1, model=MFG, iteration=1000, output = 'mean_var_norm_diff',
    ...
    if model == radial_paper:
        ...
        optimizer_radialNN = torch.optim.Adam(from_paper.parameters(), lr=0.01, amsgrad=True)
        ...
        radialNN.to(device)
        optimizer_radialNN = torch.optim.Adam(radialNN.parameters(), lr=0.01, amsgrad=True)
...
for dim in dims:
    ...
    [
       ...
       for model in [MFG, radial, radial_paper]
    ]

    N_mean_ += [N_m]
    r_mean_ += [r_m]
    r_paper_mean_ += [r_paper_m]
    N_mean, r_mean, r_paper_mean = np.stack(N_mean_),\
                                   np.stack(r_mean_),\
                                   np.stack(r_paper_mean_),

    N_var_ += [N_v]
    r_var_ += [r_v]
    r_paper_var_ += [r_paper_v]
    N_var, r_var, r_paper_var = np.stack(N_var_),\
                                np.stack(r_var_),\
                                np.stack(r_paper_var_),

    N_grad_var_ += [N_gv]
    r_grad_var_ += [r_gv]
    r_paper_grad_var_ += [r_paper_gv]
    N_grad_var, r_grad_var, r_paper_grad_var = np.stack(N_grad_var_),\
                                               np.stack(r_grad_var_),\
                                               np.stack(r_paper_grad_var_),

    np.save(
        'generated_data/blr_mean',
        np.stack([N_mean, r_mean, r_paper_mean])
    )
    np.save(
        'generated_data/blr_var',
        np.stack([N_var, r_var, r_paper_var])
    )
    np.save(
        'generated_data/blr_grad_var',
        np.stack([N_grad_var, r_grad_var, r_paper_grad_var])
    )
